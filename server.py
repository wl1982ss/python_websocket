...
def sendMessage(msg):
    ...
    back_str = []
    back_str.append('\x81')
    data_length = len(msg.encode()) #可能有中文内容传入，因此计算长度的时候需要转为bytes
    if PRINT_FLAG:
        print("INFO: send message is %s and len is %d" % (msg, len(msg.encode('utf-8'))))
    # 数据长度的三种情况
    if data_length <= 125:  #当消息内容长度小于等于125时
    ...
    def run(self):
        ...
        while True:
            if self.handshaken == False:    #如果没有进行握手
                ...
                self.buffer = self.conn.recv(1024).decode('utf-8') #socket会话收到的只能是uft-8编码的信息，将接收到的bytes数据，通过utf-8编码方式解码为unicode编码进行处理
                if PRINT_FLAG:
                    print("INFO: Socket % self.buffer is {%s}" % (self.index, self.buffer))
                if self.buffer.find('\r\n\r\n') != -1:
                    headers = {}
                    header, data = self.buffer.split('\r\n\r\n', 1) #按照这种标志分割一次，结果为:header data
                    #对header进行分割后，取出后面的n-1个部分
                    for line in header.split("\r\n")[1:]:   #再对header 和 data部分进行单独的解析
                        key, value = line.split(":", 1)     #逐行的解析Request Header信息
                        headers[key] = value
                    try:
                ...
                if g_code_length == 0:
                    get_datalength(mm)  #调用此函数可以计算并修改全局变量g_code_length 和 g_header_length
                self.length_buffer += len(mm)
                self.buffer_utf8 += mm
                if self.length_buffer - g_header_length < g_code_length:
                    if PRINT_FLAG:
                        print("INFO: 数据未接收完，继续接收")
                    continue
                else:
                    if PRINT_FLAG:
                        print("g_code_length:", g_code_length)
                        print("INFO Line 204: Recv 信息 %s, 长度为 %d:" % (self.buffer_utf8, len(self.buffer_utf8)))
...
class WebSocketServer(object):
    ...
    def begin(self):
        ...
        while True:
            ...
            #线程启动
            newSocket.start()
            #更新连接的集合（hast表的对应关系）-name->sockfd
